...
### Main Claude Code settings and features

Now, we'll go over the features and settings in the Claude Code window,
starting with those in the prompt box:
...
#### Contect with the `+` button or `@` symbol

The `+` icon lets you add chat **context in the form of files**.
While it allows you to upload files, as is common for in-browser chats,
with this kind of usage,
you should typically have the relevant files already in your workspace.
The **_Add context_** button lets you add files from your workspace as context for the chat.

![](/img/claude-context-add1.png){fig-align="center" width="40%" .lightbox fig-alt="Screenshot of the Claude Code prompt box, showing the 'Add context' button and the file selection window"}

It will really just insert an `@` symbol in the prompt box,
which triggers autocomplete to show a list of files in your workspace.
As such, the fastest way to add context is rather to **type** `@` and then select
the file or type the first few letters of the file name (it will also find files in subfolders).

![](/img/claude-context-add2.png){fig-align="center" width="70%" .lightbox fig-alt="Screenshot of the Claude Code prompt box, showing the 'Add context' button and the file selection window"}

::: callout-tip
#### Workspaces
When working with Claude Code in VS Code, you should always open a folder with
the VS Code Open Folder functionality, and have the relevant files in that folder or subfolders.
...
Typically, this will be a research project folder.
...
However, keep in mind that in general, the easiest way to add context is by simply
having the relevant file open in the editor.
Specifically, whichever file is in focus in the editor will automatically be added as context for the chat.

- [Screenshot + screenshot to turn off automatic context for the file in focus + line selection]

Finally, **your entire workspace** is what we may call the "implicit context" for the chat.
When it considers that useful for a given prompt,
Claude will often search your workspace for similar or relevant files.

### Command menu

The command menu is opened by clicking the `/`-in-a-box icon to the right of the `+`.
It has lots of functionality, including:
...
This is essentially to set the permission type Claude has to perform actions
like editing files and running commands, and can be toggled between:
...
- **Plan mode**:
  ...
  and which you can approve or (ask to) modify.
...
{{< fa user-edit >}} Switch to Auto Mode, and you should see something like the following:
...
- Claude icon will open a whole new Claude Code window,
  which is useful if you want to have multiple chats open at the same time.
...
- <kbd>Option/Alt</kbd> + <kbd>K</kbd> to persistently send a text selection to the prompt box
