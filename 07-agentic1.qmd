...
2. Once connected to OSC, click `File` > `Open Folder...`,
   and select the folder `/fs/ess/PAS3454/people/<user>`.

3. Open a terminal and make sure you are in dir `/fs/ess/PAS3454/people/<user>`:
   
   ```bash
   pwd
   ```
   ```bash-out
   /fs/ess/PAS3454/people/jane
   ```
...
3. Copy a shared data dir to your own dir:
   ...
   ```bash
   # After this, the data will be in `/fs/ess/PAS3454/people/<user>/data/iqbal`
   mkdir data
   cp -r /fs/ess/PAS3454/data/iqbal data/
   ```

   ::: {.callout-tip appearance="simple"}
   We're trying to mimick a situation where you have a self-contained research project dir,
   because:
...
4. Create a new dir `S07` (as in: session 7) within your `/fs/ess/PAS3454/people/<user>` dir:
...
3. Press <kbd>Enter</kbd> to send the request, and Copilot will provide a suggestion in the chat window:
   ...
   The above formatting with the duplicated line may be a bit confusing at first,
   ...
   code snippet below it is the suggested modified line.
