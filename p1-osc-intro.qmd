...
During the workshop, we'll work in the Project folder of the workshop's OSC Project
`PAS3454`, located at `/fs/ess/PAS3454`.
...
![A diagram showing part of the OSC file system structure, focusing on folders relevant for this workshop, for fictitious participant Jane. OSC Project-associated dirs are shown in blue and Home dirs in red. During the workshop, Jane will be working in `/fs/ess/PAS3454/people/jane`, and **not** in her Home dir `/users/PAS3454/jane`.](img/osc-paths3.png){fig-align="center" width="40%" .lightbox fig-alt="A diagram showing paths on OSC and navigating upwards with relative paths"}
...
- **By default, you will be placed in your Home directory when you --e.g.-- log in to OSC.**
  **But this is typically not where you want to be when working with data and running analyses.**
  That includes during this workshop, where you should be within `/fs/ess/PAS3454`.
...
{{< fa user-edit >}} **Click on folder `/fs/ess/PAS3454`**.
...
4. Click on the new folder and within it, create one more folder: `pre` (short for "pre-workshop").
...
- Interactive Apps like VS Code and RStudio **run on compute nodes** (not login nodes).
  ...
  - The **Working Directory** (starting location -- replace `<your-username>` with your OSC username):
    `/fs/ess/PAS3454/people/<your-username>/pre`
